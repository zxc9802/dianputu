...
主站 `SSO_USAGE_SECRETS` JSON 中的 `dianputu` 密钥必须与这里相同。不要使用 NEXT_PUBLIC_/VITE_ 前缀，不要提交真实密钥。
...
在 backend 目录运行 `python -m unittest discover -s tests -p test_main_usage.py`。

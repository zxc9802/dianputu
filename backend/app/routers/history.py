...
try:
    ...
    @router.get("")
    async def list_project_history(limit: int = 30, offset: int = 0) -> dict[str, Any]:
        """Return recent history entries (metadata only, no full project state)."""
        clamped_limit = max(1, min(limit, 100))
        clamped_offset = max(0, offset)
        items = await list_history(clamped_limit, clamped_offset)
        return {"items": items, "limit": clamped_limit, "offset": clamped_offset}

    @router.get("/{record_id}")
    async def get_project_history(record_id: str) -> dict[str, Any]:
    ...
    @router.post("")
    async def save_project_history(request: SaveHistoryRequest) -> dict[str, Any]:
        """Save or update a project history record."""
        result = await save_history(request.model_dump())
        if "error" in result:
            raise HTTPException(status_code=503, detail=result["error"])
        return result

    @router.delete("/{record_id}")
    async def delete_project_history(record_id: str) -> dict[str, Any]:
        """Delete a project history record."""
        deleted = await delete_history(record_id)
        if not deleted:

...
def _format_authority_assets(value: Any) -> str:
    ...
    if not items:
        return (
            ...
            "画面表达：真实纸质检测报告、证书纸张、装订页和泛化标签，机构名称与编号保持模糊处理"
        )
    return "\n".join(
        f"{index}. 权威方向：{_authority_asset_label(item)}；画面表达：真实纸质检测报告、证书页、纸张纹理、图表形状和泛化标签，编号、样本量、机构编号等细节保持模糊处理"
...
def _detail_text_guardrails() -> str:
    return "\n".join(
        [
            ...
            "- 报告只能做成真实纸质材质：纸张、证书页、装订报告册、桌面文件夹或盖章文件；正文必须是不可读纹理、抽象线条或极短泛化标签，不能生成可阅读的长句说明。",
        ]
    )
...
def _module_requirements(module: dict[str, Any], product_info: dict[str, Any] | None) -> str:
    ...
    requirements = {
        ...
        "authority": [
            "画面只呈现权威资质展示，核心视觉必须是真实纸质检测报告、证书页或装订报告册，不要把实验室人物当作主视觉。",
            "纸质报告要有可触摸材质：纸张纤维、轻微折痕、页脚厚度、装订线或文件夹边缘、真实桌面投影和柔和反光。",
            "报告可以摊放、叠放或被手套轻扶，搭配印章轮廓、图表形状、证书边框和少量标签；文字必须小且虚化/泛化，不写具体报告编号、样本编号、机构编号或可核验编号。",
            "实验室背景只能虚化为辅助氛围，可有显微镜、试管、玻璃器皿的浅景深轮廓，但不能盖过纸质报告主体。",
            "不要半透明 UI 卡片、悬浮数据面板、玻璃拟态卡片或电子屏幕式报告；报告必须像真实拍摄的纸张文件。",
            "必须参考以下权威资料方向，但不要把完整原文搬到画面上：",
            authority_report,
            "版式建议：桌面纸质报告特写 + 柔和实验室虚化背景 + 简洁模块标题；不做底部说明区、资料建议区或风险提示类文字。",
        ],
        "pain_scene": [
